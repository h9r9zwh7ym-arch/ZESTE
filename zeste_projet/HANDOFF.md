...
- **Numérotation des versions** : la version actuelle est **1.26**. Chaque mise à jour livrée incrémente : 1.21, 1.22, 1.23, 1.24, 1.25, 1.26… La constante est `APP_VERSION` dans `src/v117.js`. Le copyright (`COPYRIGHT`, même fichier) affiche déjà l'année en cours automatiquement (`new Date().getFullYear()`) : rien à changer chaque nouvelle année.
