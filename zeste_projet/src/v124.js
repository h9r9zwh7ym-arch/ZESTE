...
function widmarkR(){ return {h:0.68,f:0.55}[S.settings.sex]||0.66; } // facteur de Widmark ; « non précisé » = valeur moyenne
...
// inutile de reparcourir tout l'historique de l'app à chaque rendu. Chaque événement garde sa source
// (cocktail de l'historique, ou boisson ajoutée à la main) pour l'affichage détaillé et la suppression.
function bacEventsRaw(){
  const cutoff=Date.now()-30*36e5, out=[];
  S.hist.forEach(h=>{ if(h.t<cutoff) return; const r=RMAP[h.id]; if(!r||r.na) return; const m=metricsR(r); if(m.abv<=0) return; out.push({t:h.t,g:gramsOf(m.vol,m.abv),kind:"c",n:r.n}); });
  (S.drinks||[]).forEach(d=>{ if(d.t<cutoff) return; out.push({t:d.t,g:gramsOf(d.ml,d.abv),kind:"d",ref:d}); });
...
function bacState(){
  ...
  return {any:true,ready:true,active:true,perMille:cur,peak,zeroAt:now+cur/BAC_BETA*36e5};
}
function clockStr(ts){ const d=new Date(ts); return d.getHours()+"h"+(d.getMinutes()<10?"0":"")+d.getMinutes(); }
function zeroLabel(ts){ const d=new Date(ts); return d.toDateString()!==new Date().toDateString()?"demain "+clockStr(ts):clockStr(ts); }
function drinkLabel(d){ const p=BAC_PRESETS.slice().sort((a,b)=>Math.abs(a[1]-d.abv)-Math.abs(b[1]-d.abv))[0]; return (p&&Math.abs(p[1]-d.abv)<=3?p[0]:"Boisson")+", "+num(d.abv)+" %"; }

// ---------- Anneau de progression (même langage visuel que les préparations et le minuteur du mode barman) ----------
function bacRing(frac){
  const R=17,C=2*Math.PI*R;
  return `<svg class="bac-ring" viewBox="0 0 44 44"><circle cx="22" cy="22" r="${R}" fill="none" stroke="var(--fill)" stroke-width="3.5"/><circle cx="22" cy="22" r="${R}" fill="none" stroke="var(--tint)" stroke-width="3.5" stroke-linecap="round" stroke-dasharray="${(C*clamp01(frac)).toFixed(1)} ${C.toFixed(1)}" transform="rotate(-90 22 22)"/><path d="M22 13c2.6 3.7 5.6 7.3 5.6 10A5.6 5.6 0 1 1 16.4 23C16.4 20.3 19.4 16.7 22 13Z" fill="none" stroke="var(--tint)" stroke-width="1.7" stroke-linejoin="round"/></svg>`;
}
function bacRingBig(frac,label){
  const R=42,C=2*Math.PI*R;
  return `<svg viewBox="0 0 96 96" width="96" height="96"><circle cx="48" cy="48" r="${R}" fill="none" stroke="var(--fill)" stroke-width="7"/><circle cx="48" cy="48" r="${R}" fill="none" stroke="var(--tint)" stroke-width="7" stroke-linecap="round" stroke-dasharray="${(C*clamp01(frac)).toFixed(1)} ${C.toFixed(1)}" transform="rotate(-90 48 48)"/><text x="48" y="54" text-anchor="middle" font-family="ui-rounded,-apple-system,sans-serif" font-size="22" font-weight="700" fill="var(--label)">${esc(label)}</text></svg>`;
}
function bacCard(){
...
// Mise à jour toutes les minutes : tant que c'est actif on rafraîchit simplement l'onglet (coût négligeable,
// l'accroche « hero.still » évite de rejouer l'animation du verre) ; au passage à zéro, on laisse la bulle
// se refermer en douceur avant de la retirer pour de bon.
let BAC_WAS_ACTIVE=false;
setInterval(()=>{
  if(TAB!=="today") return; const st=bacState();
  if(st.active){ BAC_WAS_ACTIVE=true; dirty.today=1; renderView("today"); }
  else if(BAC_WAS_ACTIVE){ BAC_WAS_ACTIVE=false; const el=document.getElementById("bac-card");
    if(el){ el.classList.add("bac-gone"); setTimeout(()=>{ if(TAB==="today"){ dirty.today=1; renderView("today"); } },420); } }
},60000);

// ---------- Détail : ce qui compte dans l'estimation, et retirer une boisson ajoutée par erreur ----------
function bacDetailSheet(){
  openSheet(()=>{ const st=bacState(), evs=bacEventsRaw().slice().reverse();
...
Object.assign(ACT,{
  ...
  bacconfirm:()=>{ const sh=SHEETS[SHEETS.length-1]; if(!sh||!sh.bacGet) return; const {abv,ml}=sh.bacGet();
    S.drinks=(S.drinks||[]).filter(x=>x.t>Date.now()-30*36e5); S.drinks.push({id:"bd"+Date.now(),abv,ml,t:Date.now()});
    closeSheet(); changed(); toast("Boisson ajoutée"); },
  bacrm:(d)=>{ S.drinks=(S.drinks||[]).filter(x=>x.id!==d.id); changed(); refreshSheets(); }
});
