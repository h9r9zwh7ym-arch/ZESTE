...
let A11Y_Q=false;
new MutationObserver(()=>{ if(A11Y_Q) return; A11Y_Q=true; requestAnimationFrame(()=>{ A11Y_Q=false; try{ a11yPass(document.body); }catch(e){} }); }).observe(document.body,{childList:true,subtree:true});
...
function textScale(){ const v=S&&S.settings&&S.settings.txt; if(v&&v!=="auto") return +v||1;
  ...
  return 1; }
function applyTextScale(){ const k=textScale(); document.documentElement.style.fontSize=(17*k).toFixed(2)+"px"; document.documentElement.classList.toggle("txt-big",k>1.12); }
